...
export const signup = async (req, res) => {
    ...
    try{
            const exixtingAdmin= await Admin.findOne({email});
        if(exixtingAdmin){
...
export const login = async (req, res) => {
    ...
    try{
        ...
        res.status(201).json({message:"Admin logged in successfully", admin:admin,token:token});
    }catch(error){  
...
export const logout= (req,res)=>{
    ...
    try{
        if(!req.cookies?.jwt){
            return res.status(401).json({error:"Kindly login Admin jii"});
        }
         res.clearCookie("jwt");
